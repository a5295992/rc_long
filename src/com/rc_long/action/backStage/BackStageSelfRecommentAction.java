...
@Controller
public class BackStageSelfRecommentAction {
	
	

	@RequestMapping(value=AnRequest.sys_back_self_recoment)
	public ModelAndView init(HttpServletRequest req){
		...
		VideoService videoService = new VideoServiceImpl<SysVideoBean>(
				SysVideoBean.class);
		String conditionJson = "{video_type:'101',is_recommend:'1'}";
		...
		return new ModelAndView("backStage/selfRecomment/self_recomment")
	 ...
	 * @return
	 */
	@RequestMapping(value = AnRequest.sys_back_self_recoment_search)
	public ModelAndView searchVideo(HttpServletRequest req) {
		ReqUtils.Encoding(req);
		VideoService videoService = new VideoServiceImpl<SysVideoBean>(
				SysVideoBean.class);
		String conditionJson = "{video_type:'101'}";
		Map<String, String> map = new HashMap<String, String>();
		map = ReqUtils.parseSearch(map, req);
		...
		return new ModelAndView("backStage/selfRecomment/self_recomment_list")
				.addObject("self_edit_bean", self_edit_bean);
	}

	@RequestMapping(value = AnRequest.sys_back_self_recoment_update)
