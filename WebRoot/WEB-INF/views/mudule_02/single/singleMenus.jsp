		...
		<div class="w3l-medile-movies-grids">
			...
			<div class="movie-browse-agile">
				...
				<div class="browse-agile-w3ls general-w3ls">
					...
					<div class="container">
						<div class="browse-inner" id="video${menu_2.id }">

							<c:forEach items="${menu_2.videoList.list }" var="video">
				...
				<!--//browse-agile-w3ls -->
				<div class="blog-pagenat-wthree" id="wheel${menu_2.id }">
					<ul>
						<c:choose>
							<c:when test="${menu_2.videoList.pageNum == 0 }">
								<li><a class="frist" href="javascript:void(0)">上一页</a></li>
							</c:when>
							<c:otherwise>
								<li><a href="javascript:void(0)"
									onclick="lastview('${menu_2.id}','${menu_2.videoList.pageNum-1 }')">上一页</a></li>
							</c:otherwise>
						</c:choose>
						<c:choose>
							<c:when test="${menu_2.videoList.page >5 }">
								<c:set value="5" var="max"></c:set>
							</c:when>
							<c:otherwise>
								<c:set value="${menu_2.videoList.page-1}" var="max"></c:set>
							</c:otherwise>
						</c:choose>

						<c:forEach begin="${menu_2.videoList.pageNum}" end="${max }"
							var="x">
							<c:choose>
								<c:when test="${menu_2.videoList.pageNum eq x }">
									<li><a class="last" href="javascript:void(0)"
										onclick="lastview('${menu_2.id}','${x }')">${x+1 }</a></li>
								</c:when>
								<c:otherwise>
									<li><a href="javascript:void(0)"
										onclick="lastview('${menu_2.id}','${x }')"">${x+1 }</a></li>
						...
						<c:choose>
							<c:when
								test="${menu_2.videoList.pageNum eq menu_2.videoList.page }">
								<li><a class="last" href="javascript:void(0)"></a></li>
							</c:when>
							<c:otherwise>
								<li><a href="javascript:void(0)"
									onclick="lastview('${menu_2.id}','${menu_2.videoList.pageNum+1 }')">下一页</a></li>
							</c:otherwise>
						</c:choose>
					</ul>
				</div>
...
<script type="text/javascript">
	...
	function lastview(menu_id, pageNum) {
		$.ajax({
			cache : true,
			type : "GET",
			url : base + "/singles/" + menu_id + "/" + pageNum,
