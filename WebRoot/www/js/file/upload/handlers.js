...
function fileDialogComplete(numFilesSelected, numFilesQueued) {
	...
	//	alert("handlers.js中fileDialogComplete");
		this.startUpload();
	} catch (ex)  {
        this.debug(ex);
	}
...
function uploadProgress(file, bytesLoaded, bytesTotal) {
	//alert("handlers.js中uploadProcess");
	try {
...
function uploadSuccess(file, serverData) {
	//alert("handlers.js中uploadSucess");
	try {
		var progress = new FileProgress(file, this.customSettings.progressTarget);
...
//	alert("handlers.js中uploadComplete");
	if (this.getStats().files_queued === 0) {
		document.getElementById(this.customSettings.cancelButtonId).disabled = true;
	}
}
...
//	alert("handlers.js中queueComplete");
	var status = document.getElementById("divStatus");
	status.innerHTML = numFilesUploaded + " 个文件" + (numFilesUploaded === 1 ? "" : "s") + "已上传.";
		window.location.href=base+"/sys/file/swf/upload";
